...
Other tasks: `deno task build`, `deno task test`, `deno task lint`, `deno task fmt`, `deno task check`.

## Adding Device Support

Device support is defined in [src/lib/device/devices.ts](src/lib/device/devices.ts). Each device profile specifies the
product ID and available capabilities.
...
Capabilities are implemented in [src/lib/capabilities/](src/lib/capabilities/) by vendor. Each capability defines `get`
and/or `set` commands using the vendor's HID protocol.
...
3. Create a report class similar to `RazerReport` in `src/lib/device/razer/` that handles the protocol's packet
   structure, checksums, etc.
4. Add device profiles in `devices.ts` pointing to your new capabilities
5. Use [USBPcap](https://desowin.org/usbpcap/) / Wireshark to derrive the standard protocol by capturing traffic from
   the manufacturer's official software
