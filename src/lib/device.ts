...
import {
  ...
  type RazerCapabilityClasses,
  type RazerCapabilityKey,
} from './razer/index.ts'
import {
  type DiscoveredHidppCapabilities,
  type HidppCapabilityClasses,
  type HidppCapabilityKey,
} from './logitech/index.ts'
...
export type RazerDevice<K extends RazerCapabilityKey = never> =
  & Omit<
    IDevice,
...
export type HidppDevice<K extends HidppCapabilityKey = never> =
  & Omit<
    IDevice,
    'type' | 'capabilities' | 'setCapabilities'
  >
...
export type WithCapability<T, K> = T extends RazerDevice<infer Existing>
  ? K extends RazerCapabilityKey ? RazerDevice<Existing | K>
  : never
  : T extends HidppDevice<infer Existing> ? K extends HidppCapabilityKey ? HidppDevice<Existing | K>
    : never
  : never

export type Ready<T> = T extends RazerDevice<infer K> ? Omit<T, 'status' | 'failureReason' | 'capabilities'> & {
    status: 'Ready'
    failureReason: null
    capabilities: RazerCapabilitiesReady<K>
  }
  : T extends HidppDevice<infer K> ? Omit<T, 'status' | 'failureReason' | 'capabilities'> & {
      status: 'Ready'
      failureReason: null
...
type CapableOf<T, K> = T extends Ready<infer D extends DeviceType>
  ? D extends { type: infer Type extends 'razer' | 'hidpp' } ? Ready<DeviceWithCapability<Type, K>>
  : never
  : T extends DeviceType & { type: infer Type extends 'razer' | 'hidpp' } ? DeviceWithCapability<Type, K>
  : never
...
type InStatus<D, S extends DeviceStatus> = S extends 'Ready'
  ? D extends DeviceTypeMap[infer Type extends keyof DeviceTypeMap] ? Ready<DeviceTypeMap[Type]>
  : D & { status: S }
  : D & { status: S }
...
export class DeviceNotSupportedError extends Error {
  override readonly name = 'DeviceNotSupportedError'
  constructor(
    readonly vid: number,
    readonly pid: number,
  ) {
...
export class Device implements IDevice {
  @observable
  accessor status: DeviceStatus
  @observable
  accessor id: number
  @observable
  accessor failureReason: Error | null
  @observable
  accessor capabilities: Capabilities = {}
  @observable
  accessor commandErrors: CommandError[] = []
