import type { CapabilityData, UnknownCapabilities } from "./capabilities"
...
export type PossibleDeviceData<C extends CapabilityKey> =
    Pick<CapabilityData, Extract<C, keyof CapabilityData>>;
...
export function isCapableOf<
    C extends keyof Partial<UnknownCapabilities>>(device: FailedDevice, capabilities: C[]): device is FailedDeviceWithCapabilities<C>
export function isCapableOf<
    C extends keyof Partial<UnknownCapabilities>>(device: ReadyDevice, capabilities: C[]): device is ReadyDeviceWithCapabilities<C>
export function isCapableOf<
    C extends keyof Partial<UnknownCapabilities>>(device: Device, capabilities: C[]): device is DeviceWithCapabilities<C>
export function isCapableOf<
    C extends keyof Partial<UnknownCapabilities>>(device: Device, capabilities: C[]): boolean {
